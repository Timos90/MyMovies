    ...
    try:
        with connection.cursor() as cursor:
            ...
            if movies:
                ...
                print("There aren't any movies yet")
    except Error as e:
        print(f"Error retrieving movies: {e}")


def view_movie_details(connection, user_id):
    movie_id = validate_int_input("Enter the MovieID of the movie: ")
    try:
        with connection.cursor() as cursor:
            # Check if the movie is marked as deleted for the current user
            cursor.execute(
                """
                SELECT 1
                FROM deleted_movies
                WHERE user_id = %s AND movieid = %s;
            """,
                (user_id, movie_id),
            )
            user_deleted_movie = cursor.fetchone()

            if user_deleted_movie:
                print("Movie not found.")
                return

            # Check if the movie exists and is not marked as deleted
            cursor.execute(
                """
                SELECT *
                FROM movies
                WHERE movieid = %s AND deleted = FALSE;
            """,
                (movie_id,),
            )
...
def add_movie(connection, user_id):
    try:
        ...
        rating = validate_float_input(
            "Enter the rating of the movie: ", min_value=1.0, max_value=10.0
        )
        ...
        release_date = validate_date_input(
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "INSERT INTO user_movies (user_id, movieid) VALUES (%s, %s);",
                (user_id, available_id),
            )

            connection.commit()
            print("\nMovie added successfully with MovieID:", available_id)
    except Error as e:
        connection.rollback()
...
def delete_movie(connection, user_id):
    try:
        ...
        with connection.cursor() as cursor:
            # Check if the movie exists and fetch who added it
            cursor.execute(
                "SELECT added_by_user_id FROM movies WHERE movieid = %s;", (movie_id,)
            )
            added_by_user_id = cursor.fetchone()

            if not added_by_user_id:
                print(f"Movie with MovieID {movie_id} does not exist.")
                return

            if added_by_user_id[0] == user_id:
                # If the user added the movie, delete it for everyone
                cursor.execute("DELETE FROM watchlist WHERE movieid = %s;", (movie_id,))
                cursor.execute(
                    "DELETE FROM user_movies WHERE movieid = %s;", (movie_id,)
                )
                cursor.execute("DELETE FROM reviews WHERE movieid = %s;", (movie_id,))
                ...
                # Check if the user has already deleted this movie
                cursor.execute(
                    """
                    SELECT 1
                    FROM deleted_movies
                ...
                if user_deleted_movie:
                    print("You have already deleted this movie from your collection.")
                    return

                # If the user did not add the movie, soft delete it for this user
                cursor.execute(
                    "INSERT INTO deleted_movies (user_id, movieid) VALUES (%s, %s);",
                    (user_id, movie_id),
                )
                cursor.execute(
                    "DELETE FROM watchlist WHERE user_id = %s AND movieid = %s;",
                    (user_id, movie_id),
                )
                cursor.execute(
                    "DELETE FROM reviews WHERE user_id = %s AND movieid = %s;",
                    (user_id, movie_id),
                )
                cursor.execute(
                    "DELETE FROM ratings WHERE user_id = %s AND movieid = %s;",
                    (user_id, movie_id),
                )
                connection.commit()
                print("Movie deleted successfully from your collection.")
...
def rate_movie(connection, user_id):
    try:
        ...
        # Check if the movie is marked as deleted for the user
        with connection.cursor() as cursor:
            cursor.execute("""
                SELECT 1 FROM deleted_movies WHERE user_id = %s AND movieid = %s;
            """, (user_id, movie_id))
            if cursor.fetchone():
                print("You have deleted this movie from your collection and cannot rate it.")
                return

            # Check if the movieid exists in the movies table and is not marked as deleted
            cursor.execute("""
                SELECT 1 FROM movies WHERE movieid = %s AND deleted = FALSE;
            """, (movie_id,))
            if not cursor.fetchone():
                print(f"Movie with MovieID {movie_id} does not exist or has been deleted.")
                return

        # Insert into ratings if checks pass
        with connection.cursor() as cursor:
            cursor.execute("""
                INSERT INTO ratings (user_id, movieid, rating) VALUES (%s, %s, %s);
            """, (user_id, movie_id, rating))
            connection.commit()
            print("\nMovie rated successfully!!!")

            # Retrieve and display current ratings for the movie
            cursor.execute("""
            ...
            if ratings:
                print("Current Ratings for the movie:")
                for rat in ratings:
                    print(f"\nRating: {rat[0]:.1f}")
                    print(f"User: {rat[1]}")
                    print(f"Date: {rat[2]}")
...
def add_review(connection, user_id):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            """, (user_id, movie_id))
            if cursor.fetchone():
                print("You have deleted this movie from your collection and cannot add a review.")
                return
        ...
        # Insert into reviews if checks pass
        with connection.cursor() as cursor:
            cursor.execute("""
                INSERT INTO reviews (user_id, movieid, review) VALUES (%s, %s, %s);
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE r.user_id = %s AND m.deleted = FALSE
...
def add_to_watchlist(connection, user_id):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            """, (user_id, movie_id))
            if cursor.fetchone():
                print("You have deleted this movie from your collection and cannot add it to your watchlist.")
                return
            ...
            # Insert into watchlist if checks pass
            cursor.execute("""
...
def menu(connection, user_id):
    while True:
        ...
        if choice == "1":
            ...
            print("*" * 30)
            view_movie_details(connection, user_id)
            print("*" * 30)
        elif choice == "3":
            print("*" * 30)
